...
# Funzione che scansiona i dispositivi BLE, li stampa e individua quello interessato
async def scan_ble_devices(name_device='ESP32_Controller'):
    print("Scanning for BLE devices...")
    devices = await BleakScanner.discover()
    device_out = None
    for device in devices:
        print(f"Name: {device.name}, Address: {device.address}, RSSI: {device.rssi}")
        if name_device in str(device.name) :
            device_out = device
    return device_out   
...
#Scanner ble e rilavamento mac address per setup
name_ble_device = 'ESP32_Controller'

device_selected = asyncio.run(scan_ble_devices(name_device=name_ble_device))

print(f'Selezionato {device_selected.name}; address rilevato {device_selected.address}')

DEVICE_MAC_ADDRESS = device_selected.address
